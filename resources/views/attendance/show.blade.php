		    ...
		    @endif
		        ...
		        <form action="{{ route('students_attendance.create') }}" enctype="multipart/form-data">
				            ...
				            <div class="form-group d-flex text-center">
			            		<div class="form-check">
                                   @php
									...
									<input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios1" value="Regular" {{$checked}}>
									<label class="form-check-label" for="exampleRadios1">Regular</label>
								</div>
                                @if(isset($data['show']) && $data['show']==1)
    							<div class="form-check ml-2">
    								  <input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios2" value="Proxy" {{$proxy}}>
    								  <label class="form-check-label" for="exampleRadios2">Proxy</label>
    							</div>
								<div class="form-check ml-2">
									  <input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios3" value="Extra" {{$extra}}>
		            ...
		            <div class="row">
            			...
            			<div class="col-md-2">
                            <input type="hidden" id="subject_name" name="subject_name" @if(isset($data['subject_name'])) value="{{$data['subject_name']}}" @endif>
                           ...
                           @if (!empty($val))
                                ...
                                <option value="{{ $key }}" @if(isset($data['subject']) && $data['subject'] == $key) selected @endif>{{ $value }}</option>
                            @endforeach
                            @endif
                                @endforeach
                            @endif
        			     ...
        			     <div class="col-md-2" id="lecture_div">
                            <input type="hidden" id="lecture_name" name="lecture_name" @if(isset($data['lecture_name'])) value="{{$data['lecture_name']}}" @endif>
                            <input type="hidden" id="batch_id" name="batch_id" @if(isset($data['batch_id'])) value="{{$data['batch_id']}}" @endif>
                            ...
                            @if (!empty($data['all_lecture'])))
                                ...
                                <option value="{{ $value->period_id }}"  data-id="{{$value->timetable_id}}" data-batchid="{{$value->batch_ids}}" @if(isset($data['lecture']) && $data['lecture'] == $value->period_id) selected @endif>{{ $value->period_name }}</option>
                            @endforeach
                        @endif               
                            </select>
        				</div>
                        ...
                        <div class="col-md-2" id="batch_div">
                            <input type="hidden" id="batch_name" name="batch_name" @if(isset($data['batch_name'])) value="{{$data['batch_name']}}" @endif>
                ...
                @if(isset($data['student_data']))
                    ...
                    @endphp
                        <div class="card">
                            ...
                            @csrf
                            <input type="hidden" name="subjects_id" value="{{$data['subject']}}">
                            <input type="hidden" name="periods_id" value="{{$data['period_id']}}">
                            ...
                            <div class="table-responsive">
                                ...
                                <div class="row">
                                    <div class="col-md-12 form-group">
                                        <center>
                                            <input type="hidden" name="date" @if(isset($from_date)) value="{{$from_date}}" @endif">
                                            <input type="hidden" name="standard_division" @if(isset($standard_id) && isset($division_id)) value="{{$standard_id}}||{{$division_id}}" @endif">
	...
	$("#subject").empty();
      var selectedStandard = $(this).val();
       ...
       if (selectedStandard) {
            $.ajax({
                type: "GET",
                url: "/api/get-subject-list?standard_id=" + selectedStandard,
    ...
    $('#subject_name').val(name);
      ...
      // alert(selectedDate);
       if (selectedSubject) {
            $.ajax({
                type: "GET",
                url: "/get-lecture-list?subject_id=" + selectedSubject +'&standard_id='+selectedStandard+'&division_id='+selectedDivision+'&date='+selectedDate,
                success: function (res) {
                    // console.log(res);
                    ...
                    if (res.length>0) {
                       ...
                       $.each(res, function (key, value) {
                        if(value.period_name !=null){
                            $("#lecture").append('<option value="' + value.period_id + '" data-id="' + value.timetable_id + '" data-batchid="' + value.bid + '">' + value.period_name + '</option>');
                        ...
                        // Create $batch options based on batch_ids and batch_name
                            ...
                            for (var i = 0; i < batchIds.length; i++) {
                                batch.append('<option value="' + batchIds[i] + '">' + batchNames[i] + '</option>');
                            }
                        }
