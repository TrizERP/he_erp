    ...
    <div class="container-fluid">
        <div class="row bg-title">
            <div class="col-lg-3 col-md-3 col-sm-4 col-xs-12">
                <h4 class="page-title">Subject Detail to Report </h4>
            </div>
        ...
        <div class="card">
            @if ($sessionData = Session::get('data'))
                ...
                <strong>{{ is_array($sessionData['message']) ? e(implode(', ', $sessionData['message'])) : e($sessionData['message']) }}</strong>
        </div>
        @endif
        <form action="{{ route('subject_detail_attendance_report.index') }}" method="get">
            @csrf
            <div class="row">
                ...
                <table id="example" class="table display" style="border:none !important">
                    <thead>
                        <tr>
                            ...
                            <th>Enrollment No</th>
                            <th>Student Name</th>
                            @foreach ($data['week_totals'] as $monthId => $total)
                                <th> {{ $total['week_start'] }} / {{ $total['month'] }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
