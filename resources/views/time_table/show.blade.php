...
<div id="page-wrapper">
    ...
    <div class="row bg-title">
                <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                    <h4 class="page-title">Create New Time Table</h4> </div>
            </div>
									    ...
									    @foreach($data['week_data'] as $fullday => $shortday)
									        <tr>
									            ...
									            <td class="text-center" style="font-size:10px;color: black;">
									                ...
									                <select class="form-control mb-2" name="subjects[{{$value->id}}][{{$shortday}}][{{$index}}]">
										              <option>--Subject--</option>
	    												@foreach($data['subject_data'] as $sub => $subjects)
	    												@php
										                	$selected_sub=$data['old_timetable_data'][$shortday][$value->id]['SUBJECT_ID'][$index] ?? '';
										                @endphp
									                ...
									                <div class="form-group">
									                   <select class="form-control mb-2" name="teachers[{{$value->id}}][{{$shortday}}][{{$index}}]">
										              	<option>--Lecturer--</option>
    													@foreach($data['teacher_data'] as $teach => $teachers)
    													@php
									                		$selected_teach=$data['old_timetable_data'][$shortday][$value->id]['TEACHER_ID'][$index] ?? '';
									                	@endphp
									                ...
									                @if(!empty($data['batch_data']))
 													@php
									                	$selected_batch=$data['old_timetable_data'][$shortday][$value->id]['BATCH_ID'][$index] ?? '';
									                	$j++;
									                @endphp
									                <div class="form-group">
									                  <select class="form-control mb-2" name="batches[{{$value->id}}][{{$shortday}}][{{$index}}]">
										              <option>--Batch--</option>
    													@foreach($data['batch_data'] as $batch => $batches)
													  <option value="{{$batches['id']}}" @if(isset($selected_batch) && $selected_batch==$batches['id']) selected @endif>{{$batches['title']}}</option>
        										      @endforeach
														</select>
													</div>
									                @endif
									                ...
									                <!-- assigned types  -->
									                @php
									                $selected_type=$data['old_timetable_data'][$shortday][$value->id]['TYPE'][$index] ?? '';
									                @endphp
									                <div class="form-group">
									                 <select class="form-control mb-2" name="types[{{$value->id}}][{{$shortday}}][{{$index}}]">
										              <option>--Types--</option>
										              @foreach($data['types'] as $key=>$types)
										              <option value="{{$types}}" @if(isset($selected_type) && $selected_type==$types) selected @endif>{{$types}}</option>
										              @endforeach
											            </select>
											        </div>												
									            <a class="fas fa-window-close text-danger" href="#" onclick="deleteTimetable('{{$shortday.'-'.$timetable_id}}');"></a>     
									            @endforeach
									            @if(in_array($selected_type,["Lab","Tutorail"]))
									            @php 
									            $ext_lab = $data['old_timetable_data'][$shortday][$value->id]['LAB'][$index] ?? '';
									            @endphp
													 <input type="checkbox"name="extend_lab[{{$value->id}}][{{$shortday}}][{{$index}}]" @if($ext_lab=="Y") checked @endif>
												@endif
										       ...
										       @else
 												...
 												@if(!empty($data['subject_data']))
									                <div class="form-group">
										                <select class="form-control mb-2" name="subjects[{{$value->id}}][{{$shortday}}][0]">
											            <option>--Subject--</option>
	    												@foreach($data['subject_data'] as $sub => $subjects)
									            ...
									            @endif
  												...
  												@if(!empty($data['teacher_data']))
									                <div class="form-group">
									                   <select class="form-control mb-2" name="teachers[{{$value->id}}][{{$shortday}}][0]">
										              	<option>--Lecturer--</option>
    													@foreach($data['teacher_data'] as $teach => $teachers)
									            ...
									            @if(!empty($data['batch_data']))
									                <div class="form-group">
									                  <select class="form-control mb-2" name="batches[{{$value->id}}][{{$shortday}}][0]">
										              <option>--Batch--</option>
    												@foreach($data['batch_data'] as $batch => $batches)
									            ...
									            @endif
									                ...
									                <!-- unassigned type -->
									                <div class="form-group">
									                <select class="form-control mb-2" name="types[{{$value->id}}][{{$shortday}}][0]">
										              <option>--Types--</option>
										              @foreach($data['types'] as $key=>$types)
									               ...
									               <!-- add or delete timetable  -->
														...
														<a class='fas fa-plus-square' href='#' onclick="addNewRow('{{$shortday.'-'.$timetable_id}}');" id="add-{{$shortday.'-'.$timetable_id}}"></a>
									                <hr>
