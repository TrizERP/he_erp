                            ...
                            @if(count($data['data']) > 0)
                                ...
                                @foreach($data['data'] as $key => $lodata)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-inline">
                                                <a href="{{ route('lo_master.edit',['lo_master'=>$lodata->id])}}"
                                                   class="btn btn-info btn-outline"><i class="ti-pencil-alt"></i></a>
                                            </div>
                                            <form action="{{ route('lo_master.destroy', $lodata->id)}}" method="post"
                                                  class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="return confirmDelete();" type="submit"
                                                        class="btn btn-info btn-outline-danger"><i class="ti-trash"></i>
                                                </button>
                                            </form>
                                        </td>
                                    </tr>
