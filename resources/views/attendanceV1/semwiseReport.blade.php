                    ...
                    <form action="{{ route('semwise_report_v1.store') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="table-responsive">
                                ...
                                <h4
                                    style="text-align: center; font-size: 15px; font-weight: 600; font-family: Arial, Helvetica, sans-serif; margin-top: 8px;">
                                    Academic Year: {{ $syear }} - {{ $nextYear }}
                                </h4>

                                <h1
                                    style="text-align: center; font-size: 20px; margin-top: 5px; font-family: Arial, Helvetica, sans-serif; font-weight: 700;">
                                ...
                                <table id="example" class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th rowspan="2">SR No <input type="checkbox" name="checkAll" id="checkAll">
                                            </th>
                                    ...
                                    <tbody>
                                        @foreach ($data['details'] ?? [] as $val)
                                            <tr>
                                                ...
                                                {{-- ✅ Show % column only for nw --}}
                                                @if (isset($data['report_type']) && $data['report_type'] != 'pw')
                                                    <td>{{ $val['TOTAL_PERCENTAGE'] ? number_format($val['TOTAL_PERCENTAGE'], 2) : '0' }}
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
                                ...
                                <div class="signature-block"
                                    style="margin-top: 60px; padding: 20px 0; width: 100%; color:black;">
                                    <div
                        ...
                        buttons: [
                            ...
                            /* ================= CSV ================= */
                            {
                                ...
                                customize: function(csv) {
                                    ...
                                    headerRow.push('{{ App\Helpers\get_string('grno', 'request') }}');
                                    headerRow.push(
                                        '{{ App\Helpers\get_string('studentname', 'request') }}');

                                    @foreach ($data['subjects'] as $subj)
                                        headerRow.push('{{ $subj['name'] }} - L');
                                        headerRow.push('{{ $subj['name'] }} - P');
                                        headerRow.push('{{ $subj['name'] }} - T');
                                    @endforeach

                                    headerRow.push('Total L');
                                    headerRow.push('Total P');
                                    headerRow.push('Total T');
                            ...
                            /* ================= PRINT ================= */
                            {
                                ...
                                customize: function(win) {
                                    ...
                                    /* ===== Preserve original table header ===== */
                                    $(win.document.body).find('table thead').remove();
                                    $(win.document.body).find('table').prepend($('#example thead').clone());
                                    ...
                                    $(win.document.body).prepend(`
                                        ...
                                        <h4 style="text-align:center;">Academic Year: {{ $syear }} - {{ $nextYear }}</h4>
                                        <h1 style="text-align:center;">Overall Attendance Report</h1>
                                    `);

                                        /* ===== FORCE LANDSCAPE + PAGE CONTROL ===== */
