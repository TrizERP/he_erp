...
namespace App\Http\Controllers\learning_outcome\lo_greport;
...
class lo_greportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($request['id'])) {
            ...
            return is_mobile($type, 'learning_outcome/lo_greport/FinalShow', $school_data, 'view');
        } else {
            $school_data['data'] = $this->get_all_dd();
            $type = $request->input('type');

            return is_mobile($type, 'learning_outcome/lo_greport/show', $school_data, 'view');
        }
    }
    ...
    public function create(Request $request)
    {
        ...
        foreach ($sub_dd as $keys => $subject) {
            $result = DB::table("learning_outcome_indicator as LO")
                ...
                ->selectRaw('LO.SUBJECT,LO.INDICATOR,LQ.QUESTION_TITLE,sum(LQ.QUESTION_OUT_OF) tot,sum(LOM.MARKS) as mar,
            (sum(LOM.MARKS)*100/sum(LQ.QUESTION_OUT_OF)) as per')
                ->where("LQ.EXAM_TYPE", "=", $exam_type)
                ->where("LQ.medium", "=", $medium)
                ->where("LQ.subject", "=", $subject)
                ->groupBy('LO.INDICATOR')
            ...
            $result = DB::table("learning_outcome_indicator as LO")
                ...
                ->selectRaw('LO.SUBJECT,LO.INDICATOR,LQ.EXAM_CODE,SUM(LQ.QUESTION_OUT_OF) tot, SUM(LOM.MARKS) as mar,
                (SUM(LOM.MARKS)*100/ SUM(LQ.QUESTION_OUT_OF)) as per')
                ->where("LQ.EXAM_TYPE", "=", $exam_type)
                ->where("LQ.medium", "=", $medium)
                ->where("LQ.subject", "=", $subject)
    ...
    public function store(Request $request)
    {
        ...
        return is_mobile($type, 'lo_greport.index', $res, 'redirect');
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return is_mobile($type, "learning_outcome/lo_greport/edit", $data, "view");
    }
