...
class lmsCurriculumController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $res['status_code'] = 1;
        $res['message'] = "SUCCESS";
        $res['boards'] = ['GTU', 'AICTE', 'GTU-SET', 'VNSGU'];
    ...
    public function show(Request $request,$id){
        ...
        $poData = $coData = $coPoMapping = [];

        if(!empty($getData) && isset($getData)){
            $getTotalLessons = DB::table('lessonplan')->where(['standard_id'=>$getData->standard_id,'subject_id'=>$getData->subject_id,'sub_institute_id'=>$sub_institute_id,'syear'=>$syear])->count();

            $getCompletedLessons = DB::table('lessonplan')->where(['standard_id'=>$getData->standard_id,'subject_id'=>$getData->subject_id,'sub_institute_id'=>$sub_institute_id,'syear'=>$syear,'completion_status'=>'Yes'])->count();
                
            $getData->total_lesson = $getTotalLessons;
            $getData->completed_status = $getCompletedLessons;

            $poData = DB::table('lo_master')
                    ->where(['sub_institute_id'=>$sub_institute_id,'syear'=>$syear,'show_hide'=>1])
                    ->get()->toArray();

            $coData = DB::table('lo_category')
                    ->where(['standard_id'=>$getData->standard_id,'subject_id'=>$getData->subject_id,'sub_institute_id'=>$sub_institute_id,'syear'=>$syear,'show_hide'=>1])
                    ->get()->toArray();

            $coPoMapping = DB::table('tblco_po_mapping')
            ->where('sub_institute_id', $sub_institute_id)
            ->where('syear', $syear)
            ->where('standard_id', $getData->standard_id)
            ->where('subject_id', $getData->subject_id)
        ...
        // echo "<pre>";print_r($newData[0]->model_integration);exit;
        $res['status_code'] = 1;
        $res['message'] = "SUCCESS";
        $res['boards'] = ['GTU', 'AICTE', 'GTU-SET', 'VNSGU'];
        $res['model_integrations'] = ['CBE (Competency-Based Education)','PBL (Project-Based Learning)','STEAM (Science, Technology, Engineering, Arts, and Mathematics)','SEL (Social-Emotional Learning)'];
        $res['searchData'] = $getData;
        $res['coData'] = $coData;
        $res['poData'] = $poData;
        $res['coPoMapping'] = $coPoMapping;
        return is_mobile($type, 'lms/lms_curriculum/show', $res, "view");
    }
    /**
     * Remove the specified resource from storage.
     *
